...
class TestCitationAccuracy:
    ...
    def test_citation_accuracy_from_catalogue(self, chat_client):
        ...
        for test_case in test_catalogue:
            ...
            ), f"Chat request failed for query: {query}"
    ...
    @pytest.mark.parametrize(
        "test_case_index", range(5)
    )  # Test first 5 cases individually
    def test_individual_citation_cases(self, chat_client, test_case_index):
        ...
        assert (
            metrics["recall"] >= 0.90
        ), f"Citation accuracy too low: {metrics['recall']:.2%} < 90%"
...
class TestCitationFormat:
    ...
    def test_citation_response_structure(self, chat_client):
        ...
        if response.status_code == 200:
            ...
            assert (
                "citations" in data or "sources" in data or "references" in data
