    ...
    try:
        ...
        def callback(ch, method, properties, body):
            try:
                ...
                logger.info(
                    f"[TRACKING] QueryReceived: queryId={query_id}, userId={user_id}, query='{query_text}'"
                )
        ...
        logger.error("Failed to start QueryReceived consumer", exc_info=True)
...
def consume_chunks_indexed_events():
    ...
    """
    global chunks_indexed_count, documents_indexed_count, last_indexed_timestamp
    ...
    try:
        ...
        result = channel.queue_declare(queue="retrieval_chunks_indexed_metrics", durable=True)
        queue_name = result.method.queue
        ...
        def callback(ch, method, properties, body):
            ...
            try:
                ...
                correlation_id = event.get("correlationId", properties.correlation_id if properties else "unknown")
                ...
                if chunk_index == total_chunks - 1:
                    logger.info(
                        f"[MILESTONE] Document indexing completed: documentId={document_id}, "
                        f"title='{title}', "
                ...
                logger.error(
                    ...
                    extra={"correlation_id": correlation_id if 'correlation_id' in locals() else "unknown"},
                    exc_info=True
                )
                ch.basic_ack(delivery_tag=method.delivery_tag)
            except Exception as e:
                logger.error(
                    f"Error processing ChunksIndexed event: {e}",
                    extra={"correlation_id": correlation_id if 'correlation_id' in locals() else "unknown"},
                    exc_info=True
                )
                ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
        ...
        logger.error("Failed to start ChunksIndexed consumer", exc_info=True)
...
def start_consumer_thread():
    ...
    try:
        ...
        logger.error("Failed to start consumer thread", exc_info=True)
