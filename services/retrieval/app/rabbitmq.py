...
logger = setup_logger('retrieval.rabbitmq')
        ...
        for attempt in range(MAX_RETRIES):
            if not self._connect():
                if attempt < MAX_RETRIES - 1:
                    ...
                    logger.warning(f"Retrying connection in {delay:.2f}s... (attempt {attempt + 1}/{MAX_RETRIES})")
                    time.sleep(delay)
                    continue
                else:
            ...
            try:
                ...
                for routing_key, callback in self.subscriptions.items():
                    ...
                    logger.info(f"Subscribed to '{routing_key}' events on exchange '{EXCHANGE_NAME}'")
                
                logger.info(f"Starting to consume messages for {len(self.subscriptions)} event types...")
