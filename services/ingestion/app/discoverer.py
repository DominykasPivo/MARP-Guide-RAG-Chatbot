...
class MARPDocumentDiscoverer:
    ...
    def _get_document_hash(self, url: str, correlation_id: Optional[str] = None) -> str:
        ...
        try:
            response = requests.head(url, allow_redirects=True, timeout=DISCOVERY_TIMEOUT)
            ...
            logger.error(f"Failed to compute document hash for {url}: {str(e)}", extra={"correlation_id": correlation_id})
            return ""
    ...
    def discover_document_urls(self, correlation_id: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Response status: {response.status_code}", extra={"correlation_id": correlation_id})
            logger.info(f"Content length: {len(response.text)}", extra={"correlation_id": correlation_id})
            ...
            logger.error(f"Document discovery failed: {str(e)}", extra={"correlation_id": correlation_id})
            return []
    ...
    def process_documents(self, urls: List[str], correlation_id: str) -> List[DocumentDiscovered]:
        ...
        logger.info(f"Processing {len(urls)} documents.", extra={"correlation_id": correlation_id})
        discovered_docs: List[DocumentDiscovered] = []

        for url in urls:
            logger.info(f"Processing URL: {url}", extra={"correlation_id": correlation_id})
            current_hash = self._get_document_hash(url, correlation_id)
            if not current_hash:
                logger.error(f"Hash computation failed for {url}", extra={"correlation_id": correlation_id})
                continue
            logger.info(f"Hash computed: {current_hash}", extra={"correlation_id": correlation_id})

            doc_id = hashlib.sha256(url.encode()).hexdigest()
            pdf_path = os.path.join(self.storage.base_path, "documents", "pdfs", f"{doc_id}.pdf")
            ...
            if is_new_or_updated:
                if file_missing:
                    logger.info("PDF missing; re-downloading.", extra={"correlation_id": correlation_id})
                else:
                    logger.info("Document is new or updated.", extra={"correlation_id": correlation_id})
                ...
                try:
                    ...
                    logger.error(f"PDF download failed for {url}: {e}", extra={"correlation_id": correlation_id})
                    continue

                logger.info(f"PDF downloaded: {len(pdf_content)} bytes.", extra={"correlation_id": correlation_id})
                ...
                if not stored:
                    logger.error("Document storage failed.", extra={"correlation_id": correlation_id})
                    continue

                logger.info(f"Document stored: {doc_id}", extra={"correlation_id": correlation_id})
                ...
                logger.info("Document discovery event created.", extra={"correlation_id": correlation_id})
