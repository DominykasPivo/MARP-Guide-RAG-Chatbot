...
class MARPDocumentDiscoverer:
    ...
    def process_documents(self, urls: List[str], correlation_id: str) -> List[DocumentDiscovered]:
        ...
        for url in urls:
            ...
            is_new_or_updated = (
                doc_id not in self.storage.index or
                self.storage.index[doc_id].get('hash') != current_hash or
                file_missing
                )
            if is_new_or_updated:
                if file_missing:
                    logger.info(f"PDF file missing for {url}, will re-download.", extra={'correlation_id': correlation_id})
                else:
                ...
                try:
                    ...
                    logger.error(f"Failed to download PDF for {url}: {e}", extra={'correlation_id': correlation_id})
                    continue
