...
class Chunk(BaseModel):
    ...
    title: str = Field(
        ..., description="Document name for citation (e.g., 'General Regulations')."
    )
    ...
    score: Optional[float] = Field(
        default=0.0, description="Relevance score of the chunk."
    )
...
class LLMResponse(BaseModel):
    ...
    model: str = Field(
        ..., description="Name of the LLM model used (e.g., 'claude-3.5-sonnet')."
    )
    answer: str = Field(..., description="The generated answer from this model.")
    citations: List[Citation] = Field(
        default=[], description="Citations used for this answer."
    )
    generation_time: float = Field(
        ..., description="Time taken to generate this response in seconds."
    )
...
class ChatResponse(BaseModel):
    ...
    responses: List[LLMResponse] = Field(
        ..., description="Responses from different LLM models."
    )
