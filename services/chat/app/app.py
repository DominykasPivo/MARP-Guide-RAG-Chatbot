...
from llm_rag_helpers import (
    generate_answers_parallel,
)
from models import ChatResponse, Chunk, LLMResponse
...
async def chat(request: Request, chat_request: ChatRequestModel):
    ...
    try:
        ...
        logger.info(
            f"💬 Received chat request: '{query}' (correlation_id: {correlation_id})"
        )
        ...
        logger.info(
            f"🤖 Generating answers from {len(LLM_MODELS)} models " "in parallel..."
        )
        llm_responses = await generate_answers_parallel(
            query, chunks, api_key=OPENROUTER_API_KEY, models=LLM_MODELS
        )

        logger.info(
            f"✅ Generated {len(llm_responses)} responses from different models"
        )
