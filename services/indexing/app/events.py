...
def process_extracted_event(message):
    ...
    from rabbitmq import EXCHANGE_NAME, pika
    ...
    # logger.info(f"DEBUG: all_chunks count for {document_id}:
    #             {len(all_chunks)}")
    # for idx, chunk in enumerate(all_chunks):
    #     logger.info(f"DEBUG: all_chunks[{idx}] chunk_index={{}}
    #                 text_len={{}}".format(
    ...
    # logger.info(f"DEBUG: unique_chunks count for {document_id}:
    #             {len(chunks)}")
    # for idx, chunk in enumerate(chunks):
    #     logger.info(f"DEBUG: unique_chunks[{idx}] chunk_index={{}}
    #                 text_len={{}}".format(
    #                 chunk['metadata'].get('chunk_index'),
    #                 len(chunk['text'])))
    ...
    #                 text_length={len(first_chunk.get('text', ''))}",
    #                 extra={"correlation_id": correlation_id})

    embedded_chunks = embed_chunks(chunks, correlation_id=correlation_id)

    # ADD DEBUG: Check what embedded chunks look like
    # logger.info(f"📊 Generated {len(embedded_chunks)} "
    #             f"embedded chunks",
    #             extra={"correlation_id": correlation_id})
    # if embedded_chunks:
    #     first_embedded = embedded_chunks[0]
    #     logger.info(f"🔍 First embedded chunk:
    #                 has_text={bool(first_embedded.get('text'))},
    #                 has_embedding="
    #                 f"{bool(first_embedded.get('embedding'))},
    #                 has_metadata="
    #                 f"{bool(first_embedded.get('metadata'))}",
    #                 extra={"correlation_id": correlation_id})
    #     if first_embedded.get('embedding'):
    #         logger.info(f"🔍 Embedding length: "
    #                     f"{len(first_embedded['embedding'])}",
    #                     extra={"correlation_id": correlation_id})
    ...
    #     logger.info(
    #         f"📝 Chunk {idx}: id={chunk_id}, meta={json.dumps(meta)},
    ...
    store_chunks_in_qdrant(
        embedded_chunks, collection_name="chunks", correlation_id=correlation_id
    )
    logger.info(
        f"✅ Stored {len(embedded_chunks)} chunks in Qdrant",
        extra={"correlation_id": correlation_id},
    )
    ...
    try:
        ...
        connection.close()
        logger.info(
            f"📤 Published {published_count} ChunksIndexed events "
            f"for document {document_id}",
            extra={"correlation_id": correlation_id},
        )
    except Exception as e:
        logger.error(
            f"❌ Failed to publish ChunksIndexed events: {e}",
            extra={"correlation_id": correlation_id},
            exc_info=True,
        )
